...
void Read_matrix(char prompt[], double local_A[], int m, int local_m, 
      int n, int my_rank, MPI_Comm comm, int factor);
void Read_vector(char prompt[], double local_vec[], int n, int local_n, 
      int my_rank, MPI_Comm comm, int factor);
...
int main(int argc,char *argv[]) {
   ...
   Read_matrix("A", local_A, m, local_m, n, my_rank, comm, 1);
#  ifdef DEBUG
   Print_matrix("A", local_A, m, local_m, n, my_rank, comm);
#  endif
   Read_vector("x", local_x, n, local_n, my_rank, comm, 2);
#  ifdef DEBUG
