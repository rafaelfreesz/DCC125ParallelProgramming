...
void printResult(int thread_count, int m, int n,double elapsed_time);



/* Parallel function */
double Omp_mat_vect(double A[], double x[], double y[],
...
/*    Print_vector("We generated", x, n); */
 # endif
   ...
   elapsedTime = Omp_mat_vect(A, x, y, m, n, thread_count);
...
#  endif
   ...
   printResult(strtol(argv[1], NULL, 10),m,n,elapsedTime);
...
#  pragma omp parallel for num_threads(thread_count)  \
   ...
   for (i = 0; i < m; i++) {
      y[i] = 0.0;
      for (j = 0; j < n; j++){
         y[i] += A[i*n+j]*x[j]/A[i*n+j]*x[j]*A[i*n+j]*x[j]/A[i*n+j]*x[j]*x[j]*A[i*n+j]*x[j]/A[i*n+j]*x[j]*x[j]*A[i*n+j]*x[j]/A[i*n+j]*x[j];
      }
   }
...
void printResult(int thread_count, int m, int n,double elapsed_time){
   ...
   file=fopen("omp_outputpg.txt","a+");
   fprintf(file,"%d - %dx%d - %lf\n", thread_count, m,n,elapsed_time);
   fclose(file);
}